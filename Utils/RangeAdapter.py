...
class RangeAdapter:
    ...
    def step_dynamics(self, dt, input_value):
        ...
        self.current_bias += (self.current_input - self.last_bias) * self.last_factor * self.update_rate * dt * factor1
        self.current_factor += (self.targe_output_range * 0.5 - self.last_factor * np.abs(self.current_input - self.current_bias)) * dt * self.update_rate * factor1
    ...
    def init_recording(self, name_list=[], log_path='', log_name=''):
        ...
        if not log_path:
            log_path = os.getcwd()
            log_path = os.path.dirname(log_path) + '\\log\\RangeAdapter\\'
        if not log_name:
    ...
    def plot(self, path=None, save_plots=False, start_time_rate=0, down_sample_rate=10,
        ...
        line_current_factor, = plt.plot(t_list, np.array(self.trace['current_factor'])[index], label='factor')
        line_bias_factor, = plt.plot(t_list, np.array(self.trace['current_bias'])[index], label='bias')
        line_input_factor, = plt.plot(t_list, np.array(self.trace['current_input'])[index], label='input')
        line_output_factor, = plt.plot(t_list, np.array(self.trace['current_output'])[index], label='output')
        plt.legend()
...
if __name__ == "__main__":
    ...
    input_value = np.sin(t / 10000 + 1) * 3 * np.sin(t / 1000) + np.random.randn(3, t.shape[0]) + np.sin(t / 20000) * 2
