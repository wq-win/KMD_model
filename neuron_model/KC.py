...
class KC_Neuron(Neuron):
    def __init__(self, phi=0.1, threshold=0.5):
        # todo 修改初始化赋值
        super().__init__(phi, threshold)
        self.K = 1  # 当前兴奋程度
        self.active_MBON = False
        self.active_DAN = False

    def derivative(self, state, inputs=0):
        phi = state
        Dphi = self.alpha * self.K - self.beta * phi
        return Dphi
...
phi = 0.1
threshold = 0.5
KC1 = KC_Neuron(phi=phi, threshold=threshold)
phi_state = []
for t in times:
